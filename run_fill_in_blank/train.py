...
def evaluate(model, dataloader):
    ...
    for v, q, a, qid in iter(dataloader):
        with torch.no_grad():
            v = v.to(device)
            q = q.to(device)
            a = a.to(device)
...
def train(model, train_loader, eval_loader, num_epochs, output, save_all, patience, lr):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (v, q, a, pid) in enumerate(train_loader):
            v = v.to(device)
            q = q.to(device)
            a = a.to(device)
...
def parse_args():
    ...
    parser.add_argument('--output', type=str, default='../saved_models/fill_in_blank')
    # data splits
    ...
    parser.add_argument('--max_length', type=int, default=34, help='34 for fill_in_blank task')
