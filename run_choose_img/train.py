...
def train(model, train_loader, eval_loader, num_epochs, output, save_all, patience, lr):
    ...
    for epoch in range(num_epochs):
        ...
        if patience != -1 and epoch > max(20, patience) and max(eval_scores[-patience:]) < best_eval_score: # last N epochs
...
def parse_args():
    ...
    parser.add_argument('--task', type=str, default='choose_img')
    parser.add_argument('--save_all', type=bool, default=False, help='save all model checkpoints or not')
    parser.add_argument('--model_input', type=str, default='../saved_models/choose_img')
    ...
    parser.add_argument('--choice_feat', type=str, default='resnet101_pool5_icon',
                        help = 'resnet101_pool5_79_icon: icon pretrained model')

    parser.add_argument('--input', type=str, default='../data')
    parser.add_argument('--output', type=str, default='../saved_models/choose_img')
    # data splits
    ...
    parser.add_argument('--max_length', type=int, default=71, help='71 for choose_img task')
...
if __name__ == '__main__':
    ...
    train_dset = IconQAFeatureDataset(args.train_split, args.task, args.feat_label, args.choice_feat, args.input,
                                   dictionary, args.lang_model, args.max_length, args.num_patches) # generate train data
    eval_dset = IconQAFeatureDataset(args.val_split, args.task, args.feat_label, args.choice_feat, args.input,
                                  dictionary, args.lang_model, args.max_length, args.num_patches) # generate val data
